...
def get_payments(
    ...
    return db.query(Payment).join(Contract).filter(Contract.id_арендатора == current_user.id).all()
...
def get_payment(
    ...
    if current_user.role == "tenant" and payment.договор.id_арендатора != current_user.id:
...
def check_overdue_payments(
    ...
    today = date.today()
    overdue = db.query(Payment).filter(Payment.дата_платежа < today, Payment.статус == "не оплачен").all()
