        ...
        {offices.map((office) => (
          <div key={office.id_офиса} className="bg-white rounded-lg shadow-md p-6 border border-gray-200">
            <div className="flex justify-between items-start mb-4">
              <h3 className="font-semibold text-lg text-gray-800">{office.адрес || 'Без адреса'}</h3>
            ...
            <div className="space-y-2 text-sm text-gray-600">
              ...
              <p>Комнат: {office.количество_комнат || 0}</p>
              <p className="font-semibold text-gray-800">
                {office.стоимость_аренды ? office.стоимость_аренды.toLocaleString() : '0'} ₽/мес
              </p>
            </div>
            {user?.role === 'admin' && (
                ...
                >
                  ...
                  {offices.filter(o => o.статус === 'свободен').map((office) => (
                    <option key={office.id_офиса} value={office.id_офиса}>
                      {office.адрес} - {office.стоимость_аренды ? office.стоимость_аренды.toLocaleString() : '0'} ₽/мес
                    </option>
                  ))}
                </select>
              </div>
